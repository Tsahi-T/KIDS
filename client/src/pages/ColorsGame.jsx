...
import { generateColorQuestion } from '../utils/colors.js'
...
export default function ColorsGame({ player, onGameOver }) {
  ...
  function nextQuestion() {
    ...
    setSelected(null)
    if (usedRef.current.size >= 12) usedRef.current.clear()
    const q = generateColorQuestion(usedRef.current)
  ...
  function handleTimeout() {
    ...
    setSelected({ chosen: null, correctWord: question.correct.word })
    advance()
  }
  ...
  const handleAnswer = useCallback((word) => {
    ...
    setSelected({ chosen: word, correctWord: question.correct.word })
  ...
  const timerColor = remaining > 8 ? '#4ade80' : remaining > 4 ? '#fbbf24' : '#f87171'
  const urgent     = remaining <= 4

  const isColorToWord = question.mode === 'color-to-word'
  ...
  return (
    <div className="eng-screen">
      ...
      <div className="eng-hint">
        {isColorToWord ? 'מה שם הצבע באנגלית?' : 'בחר את הצבע הנכון'}
      </div>
      ...
      <div className="eng-grid">
        {question.answers.map((item, i) => {
          let state = ''
          if (selected) {
            if (item.word === selected.correctWord) state = ' ans-correct'
            else if (item.word === selected.chosen)  state = ' ans-wrong'
          }

          return isColorToWord ? (
            /* word buttons */
            <button
              key={i}
              className={`eng-btn color-word-btn${state}`}
              onClick={() => handleAnswer(item.word)}
              disabled={!!selected}
            >
              <span className="color-word-ans">{item.word}</span>
            </button>
          ) : (
            /* color swatch buttons */
            <button
              key={i}
              className={`eng-btn color-swatch-btn${state}`}
              onClick={() => handleAnswer(item.word)}
              disabled={!!selected}
